...
- A worker that is to be stopped is asked to exit over its own connection (a `stop` command, which a CLI of this version acts on), and its launcher is only asked to stop it if it isn't connected or hasn't gone within 30 seconds. So a worker leaves cleanly even when the launcher can't be reached — with expired credentials, say — and the launcher still bounds a worker that doesn't respond.
...
- In the pool topic, a worker's `stoppedAt` is set when its launcher confirms it has gone, having been asked to stop, rather than when the launcher accepted the request — a task that is still stopping is no longer shown as stopped. `stoppingAt` is when the worker was first asked (over its connection or through the launcher), and `stopError` is how the latest attempt went.
