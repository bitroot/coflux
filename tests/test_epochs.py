...
def test_asset_reference_across_epoch_boundary(isolated_server, tmp_path):
    ...
    with managed_worker(targets, host, tmp_path, concurrency=2) as executor:
        ...
        original_entry = next(iter(original_entries.values()))
        ...
        rerun_entry = next(iter(rerun_entries.values()))
...
def test_idempotency_across_epoch_boundary(isolated_server, tmp_path):
    ...
    with managed_worker(targets, host, tmp_path) as executor:
        ...
        resp1 = cli.submit("test/my_workflow", idempotency_key="epoch-key", host=host)
        run_id1 = resp1["runId"]
        ...
        resp2 = cli.submit("test/my_workflow", idempotency_key="epoch-key", host=host)
        run_id2 = resp2["runId"]
